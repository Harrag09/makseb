...
const generateTicketsHTML = async (req, res) => {
    try {
        ...
        data.Menu.forEach(item => {
            ...
            htmlContent += `
            <li>${item.QtyProduct}. ${item.NameProduct}: ${item.QtyProduct * item.TTC} ${data.devise}</li>`;
            ...
            if (item.Gredient && item.Gredient.length > 0) {
                item.Gredient.forEach(option => {
                    ...
                    htmlContent += `
                    <li>${option.NameProduct}: ${option.TTC * option.QtyProduct} ${data.devise}</li>`;
                });
            }
            ...
            if (item.Sup && item.Sup.length > 0) {
                item.Sup.forEach(option => {
                    ...
                    totalTVA += option.TVA * option.QtyProduct;
                    htmlContent += `
                    <li>${option.QtyProduct}. ${option.NameProduct}: ${option.TTC * option.QtyProduct} ${data.devise}</li>`;
                });
            }
        });
        ...
        htmlContent += `
                ...
                <div class="payment-details">
                    <p>MONTANT HT: ${totalHT.toFixed(1)}${data.devise}</p>
                    <p>TOTAL: ${data.TTC} ${data.devise}</p>
                    <p>DONT TVA: ${totalTVA.toFixed(1)}${data.devise}</p>`;
        
        data.ModePaiement.forEach(payment => {
            htmlContent += `
            <p>${payment.ModePaimeent}: ${payment.totalwithMode} ${data.devise}</p>`;
        });
        ...
        console.error("Error generating tickets HTML:", error);
        res.status(500).send("Error generating tickets HTML");
    }
};
